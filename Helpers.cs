...
namespace _260328_ResiCalc
{
    internal static class Helpers
    {
        ...
        public static readonly double[] E24Base =
        {
            1.0, 1.1, 1.2, 1.3, 1.5, 1.6, 1.8, 2.0, 2.2, 2.4, 2.7, 3.0,
            3.3, 3.6, 3.9, 4.3, 4.7, 5.1, 5.6, 6.2, 6.8, 7.5, 8.2, 9.1
        };

        public static readonly double[] E96Base =
        {
        ...
        public static readonly double[] E192Base =
        {
        ...
        // E12 인덕터 기준값
        public static readonly double[] E12Base =
        {
        ...
        /// <summary>기준값 배열을 지정 디케이드 범위로 확장</summary>
        public static double[] ExpandDecades(double[] baseValues, int expMin, int expMax)
        {
            var list = new List<double>(baseValues.Length * (expMax - expMin + 1));
            for (int e = expMin; e <= expMax; e++)
            {
                double m = Math.Pow(10, e);
                foreach (double v in baseValues)
                    list.Add(v * m);
            }
            return list.ToArray();
        }

        /// <summary>로그 스케일 기준으로 series 내 target에 가장 가까운 값 반환</summary>
        public static double FindNearest(double[] series, double target)
        {
            double best = series[0], bestDist = double.MaxValue;
            double logT = Math.Log(target);
            foreach (double v in series)
            {
                double d = Math.Abs(Math.Log(v) - logT);
                if (d < bestDist) { bestDist = d; best = v; }
            }
            return best;
        }

        /// <summary>저항값을 Ω / kΩ / MΩ 단위로 포맷</summary>
        public static string FormatR(double ohms)
        {
            if (ohms >= 1_000_000) return $"{ohms / 1_000_000:0.##} MΩ";
            if (ohms >= 1_000)     return $"{ohms / 1_000:0.##} kΩ";
            return $"{ohms:0.##} Ω";
        }

        /// <summary>인덕턴스를 nH / μH / mH / H 단위로 포맷</summary>
        public static string FormatL(double henry)
        {
            if (henry >= 1.0)  return $"{henry:0.###} H";
            if (henry >= 1e-3) return $"{henry * 1e3:0.###} mH";
            if (henry >= 1e-6) return $"{henry * 1e6:0.###} μH";
            return $"{henry * 1e9:0.###} nH";
        }

        /// <summary>소수점 구분자를 무시하고 double 파싱</summary>
        ...
        public static void ShowError(string msg) =>
            System.Windows.MessageBox.Show(msg, "입력 오류",
